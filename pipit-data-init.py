...
names = pipit_train.columns[1:].tolist()
...
for i in range(len(dataset_train)):
    ...
    if dataset_train[i]['labels'] in cache.keys():
        ...
        torch.save(dataset_train[i]['input_values'], f'./dataset-input-pipit/Train/audio_arrays_{start}_' + label_name + '.pt')
        start += 1
    else:
...
for i in range(len(dataset_test)):
    ...
    if dataset_test[i]['labels'] in cache.keys():
        ...
        torch.save(dataset_test[i]['input_values'], f'./dataset-input-pipit/Test/audio_arrays_{start}_' + label_name + '.pt')
    else:
