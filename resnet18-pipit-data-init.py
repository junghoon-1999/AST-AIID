...
train_rec = pd.read_csv('./Bird-audio/csv/pipit-withinyear-fg-trn.csv')
test_rec = pd.read_csv('./Bird-audio/csv/pipit-withinyear-fg-tst.csv')
...
for i in range(len(train_rec)):
    for n in train_rec.columns[1:]:
        if train_rec[n].iloc[i] == 1:
            ...
            audio_file = join('./Bird-audio/pipit-fg/', filename)
            ...
            destination = join('./cnn-pipit-data/Train', str(n))
...
for i in range(len(test_rec)):
    for n in test_rec.columns[1:]:
        if test_rec[n].iloc[i] == 1:
            ...
            # Splits the name of the file to find what folder it should save it in
            destination = join('./cnn-pipit-data/Test', str(n))
